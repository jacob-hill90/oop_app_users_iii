...
class User:
    ...
    def create_post(self, title, body, time):
        if self.tier == "free" and self.post_count > 1:
            print("Number of daily posts exceeded")
            return "Number of daily posts exceeded"
        ...
        if self.name in User.posts_by_user_dict:
            User.posts_by_user_dict[self.name].append(post.create_str())
        else:
            User.posts_by_user_dict[self.name] = [post.create_str()]
...
# jake.create_post("Post 1", "I'm hungry", "10:15")
# jake.create_post("Post 2", "I'm tired", "11:19")
# jake.create_post("Post 3", "I'm angry", "12:30")
# kate.create_post("Post 4", "I'm happy", "4:15")
